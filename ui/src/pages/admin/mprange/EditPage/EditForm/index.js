...
import { REDUCER, getPointsDetail, addPoints, editPoints } from 'ducks/mprange';
		...
		if (!isSubmitForm) {
			form.validateFields((error, values) => {
				if (!error) {
					if (isAdd) {
						values['type'] = type;
						dispatch(addPoints(values));
					} else {
						dispatch(editPoints(type, values));
					}
				}
			});
		}
